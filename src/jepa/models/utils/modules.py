...
try:
    from timm.layers import drop_path as timm_drop_path
except ImportError:
    try:
        from timm.models.layers import drop_path as timm_drop_path
    except ImportError:
        def timm_drop_path(x, drop_prob: float = 0.0, training: bool = False):
...
def _sdpa_context():
    if hasattr(torch.nn, "attention") and hasattr(torch.nn.attention, "sdpa_kernel"):
        ...
        return torch.nn.attention.sdpa_kernel(backends=backends)
